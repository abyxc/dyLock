...
<html>
     <head>
  	...
  	<body>
  		<div class="nav-header">
		    <p class="new-add">后台用户&gt;&gt;修改</p>
		</div>
  			<div class="look-table-container">
  			<form action="${basePath}/manage/user/update.action" method="post" id="Form">
  				<input type="hidden" name="user.id" value="${user.id }">
  				<input type="hidden" name="user.role_type" value="${user.role_type }">
				<input type="hidden" name="user.create_time" value="${user.create_time }">
				<input type="hidden" name="user.openid" value="${user.openid }">
  				<table cellpadding="0" cellspacing="0" class="look-table">
  					<tr>
  						<td><span class="name-title">名称：</span><s:textfield name="user.name" id="name" cssStyle="width: 200px;height: 25px;"></s:textfield></td>
  						<td><span class="name-title">性别：</span><s:radio list="#{'0':'男','1':'女'}" name="user.sex" id="sex"></s:radio></td>
			    	</tr>
  					<tr>
  						...
  						<td><span class="name-title">角色：</span><s:select id="role_id" list="groups" name="user.role_id" listKey="id" listValue="name" headerKey="" headerValue="--请选择--" cssStyle="width: 200px;"></s:select></td>
			    	</tr>
  					<tr>
  	...
  	<script type="text/javascript">
    	function ajaxLoginname(newLoginName,oldLoginName){
     		if(newLoginName == oldLoginName){
     			return false;
     		}
     		if(newLoginName.trim().length == 0){
         		return false;
     		}
    ...
    <script type="text/javascript">
  		function tijiao(tearm){
			...
			//有一个事件触发事件可以兼容ie
		  	if(confirm('确认提交？')){
			  	openlink('${basePath}/manage/user/update.action?searchTearm1=${searchTearm1}&searchTearm2=${searchTearm2 }&pageBean.currentPage=${pageBean.currentPage}','Form',1);
