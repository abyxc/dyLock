...
// 短信内容
	...
	<div class="nav-header">
	    <p class="new-add">模板列表&gt;&gt;修改</p>
	</div>
	<div class="look-table-container">
		<form action="${basePath}/manage/templet/update.action" method="post" id="Form">
		    <s:hidden name="templet.id"></s:hidden>
		    <s:hidden name="templet.state"></s:hidden>
	  ...
	  function insert(){
		...
		if(content.indexOf("姓名")>0){
	  	...
	  	if(content.indexOf("联系电话")>0){
			...
			//有一个事件触发事件可以兼容ie
		  	if(confirm('确认提交？')){
			  	openlink('${basePath}/manage/templet/update.action?searchTearm1=${searchTearm1 }&pageBean.currentPage=${pageBean.currentPage}','Form',1);
